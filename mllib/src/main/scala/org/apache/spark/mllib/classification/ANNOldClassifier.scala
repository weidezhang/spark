...
class ANNOldClassifierModel private[mllib](val annModel: ArtificialNeuralNetworkModel,
                                        val labelToIndex: Map[Double, Int])
  extends ClassificationModel with ANNOldClassifierHelper with Serializable {
  /**
...
class ANNOldClassifier private(val labelToIndex: Map[Double, Int],
  ...
  extends ANNOldClassifierHelper with Serializable {

  def run(data: RDD[LabeledPoint], batchSize: Int = 1): ANNOldClassifierModel = {
    ...
    new ANNOldClassifierModel(model, labelToIndex)
  }
}
...
object ANNOldClassifier {
  ...
  def train(data: RDD[LabeledPoint],
            ...
            maxIterations: Int,
            convergenceTol: Double): ANNOldClassifierModel = {
  ...
  def train(data: RDD[LabeledPoint],
    ...
    new ANNOldClassifier(labelToIndex, hiddenLayersTopology,
   ...
   * @return ANN classifier model.
   */
  def train(data: RDD[LabeledPoint]): ANNOldClassifierModel = {
  ...
  def randomWeights(data: RDD[LabeledPoint],
    ...
    /* TODO: remove duplication - the same analysis will be done in ANNOldClassifier.run() */
