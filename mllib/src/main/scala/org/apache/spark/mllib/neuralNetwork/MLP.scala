...
class MLP(
  ...
  val hiddenLayersDropoutRate: Double = 0.5D,
  val inputLayerDropoutRate: Double = 0.2D) extends Logging with Serializable {
...
object MLP {
  ...
  def runSGD(
    ...
    runSGD(trainingRDD, nn, batchSize, maxNumIterations,
      fraction, regParam, learningRate)
  }
  ...
  def runSGD(
    ...
    val updater = new MLPAdaDeltaUpdater(nn.topology)
  ...
  private[mllib] def initializeLayers(topology: Array[Int]): Array[Layer] = {
    ...
    for (layer <- (0 until numLayer).reverse) {
      layers(layer) = if (layer == numLayer - 1) {
        ...
        new SoftPlusLayer(topology(layer), topology(layer + 1))
      }
...
private[mllib] class MLPAdaGradUpdater(
  ...
  rho: Double = 0,
  gamma: Double = 1e-1,
  epsilon: Double = 1e-2) extends MLPUpdater(topology) {
  assert(rho >= 0 && rho < 1)
  lazy val etaSum = {
...
private[mllib] class MLPAdaDeltaUpdater(
  topology: Array[Int],
  rho: Double = 0.99,
  epsilon: Double = 1e-8) extends MLPUpdater(topology) {
