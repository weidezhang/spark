...
object DataValidators extends Logging {
   ...
   */
  val binaryLabelValidator: RDD[LabeledPoint] => Boolean = multinomialLabelValidator(2)

  /**
   * Function to check if labels used for k classes classification are
   * in the range of [0, 1, ..., k-1].
   *
   * @return True if labels are all in the range of [0, 1, ..., k-1], false otherwise.
   */
  def multinomialLabelValidator(k: Int): RDD[LabeledPoint] => Boolean = { data =>
    val numInvalid = data.filter(x => {
      ((x.label - x.label.toInt) != 0) || (x.label < 0) || (x.label > k - 1)}).count()
    if (numInvalid != 0) {
